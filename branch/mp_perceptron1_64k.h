...
#define MAX_ASSOC 256
	...
	void analyze_spec(void)
	{
		...
		for (int i = 0; i < num_tables; i++)
		{
			...
			if (specs[i].type == LOCAL)
				doing_local = true;

			// how many IMLI counter bits do we need for different versions of IMLI

			if (specs[i].type == IMLI)
				imli_counter_bits[specs[i].p1 - 1] = 32;
			...
			if (specs[i].type == BLURRYPATH)
				for (int j = 0; j < specs[i].p2; j++)
					blurrypath_bits[specs[i].p1][j] = 32 - specs[i].p1;
			...
			if (specs[i].type == MODHIST || specs[i].type == GHISTMODPATH)
			{
				...
				if (modhist_lengths[j] < specs[i].p2 + 1)
					modhist_lengths[j] = specs[i].p2 + 1;
				if (specs[i].p2 >= modghist_length)
					modghist_length = specs[i].p2 + 1;
			}
		}
		...
		for (int i = 0; i < num_tables; i++)
		{
			if (specs[i].type == MODPATH || specs[i].type == GHISTMODPATH)
			{
				...
				if (modpath_lengths[j] < specs[i].p2 + 1)
					modpath_lengths[j] = specs[i].p2 + 1;
				if (path_length <= specs[i].p2)
					path_length = specs[i].p2 + 1;
			}
			if (specs[i].type == PATH)
			{
				if (path_length <= specs[i].p1)
					path_length = specs[i].p1 + 1;
			}
		}
		...
		for (int i = 0; i < num_tables; i++)
		{
			...
			{
			case GHIST:
				if (ghist_length <= specs[i].p2)
					ghist_length = specs[i].p2 + 1;
				break;
			case GHISTPATH:
				if (ghist_length <= specs[i].p1)
					ghist_length = specs[i].p1 + 1;
				if (path_length <= specs[i].p1)
					path_length = specs[i].p1 + 1;
				break;
			default:;
		...
		for (int i = 0; i < nmodhist_histories; i++)
			totalbits += modhist_lengths[i];

		// account for modulo path bits

		for (int i = 0; i < nmodpath_histories; i++)
			totalbits += 16 * modpath_lengths[i];

		// account for local histories

		if (doing_local)
			totalbits += local_history_length * nlocal_histories;
		...
		for (int i = 0; i < MAX_BLURRY; i++)
			for (int j = 0; j < MAX_BLURRY2; j++)
				totalbits += blurrypath_bits[i][j];
	...
	multiperspective_perceptron(
			...
			unsigned long long int _imli_mask4 = 0, unsigned long long int _recencypos_mask = 0) : specs(_specs), num_tables(_num_tables), budgetbits(_budgetbits), nlocal_histories(_nlocal_histories),
	...
	void findbest(int *bestpreds)
	{
		...
		for (int i = 0; i < nbest; i++)
			bestpreds[i] = pairs[i].index;
	}

	unsigned int get_index(int i)
	{
		...
		if ((1ull << i) & imli_mask1)
			h ^= imli_counter1;
		if ((1ull << i) & imli_mask4)
			h ^= imli_counter4;
		if ((1ull << i) & recencypos_mask)
			h ^= hash_recencypos(u.pc2, 31, i);
		h %= table_sizes[i];
		return h;
	}
	...
	branch_info *lookup(unsigned int pc)
	{
		...
		if (num_filter)
		{
			...
			if (!(f->seen_taken) && !(f->seen_untaken))
				use_static = true;
		}
	...
	void update(branch_info *p, unsigned int target, bool taken, int type)
	{
		...
		if (num_filter)
		{
			...
			if (decay && transition && ((occupancy > decay) || (decay == 1)))
			{
				...
				if (filter_table[r].seen_taken && filter_table[r].seen_untaken)
					occupancy--;
...
history_spec big[37] = {
		...
		{GHISTMODPATH, 1, 5, 4, 1.450000, 1913, 6},
		...
		{IMLI, 4, -1, -1, 1.78125, 1536, 6},
		{LOCAL, -1, -1, -1, 1.0, 1024, 6},
		{LOCAL, -1, -1, -1, 2.0, 3467, 6},
		{MODHIST, 1, 16, -1, 0.9375, 0, 6},
		{MODPATH, 3, 20, 1, 1.0, 0, 6},
		{PATH, 13, 2, -1, 1.4375, 0, 6},
		{PATH, 27, 5, -1, 1.00000, 0, 6},
		{RECENCY, 10, 3, -1, 0.550000, 1024, 6},
		{RECENCY, 14, 4, -1, 1.3125, 0, 6},
		{RECENCYPOS, 31, -1, -1, 1.5, 0, 6},
		{SGHISTPATH, 1, 2, 5, 1.25, 768, 6},
		{SGHISTPATH, 1, 5, 2, 1.3125, 972, 6},
};

PREDICTOR::PREDICTOR(void)
{
	static int branchyxlat[] = {
			1,
			3,
			4,
			5,
	...
	ex = new multiperspective_perceptron(
			...
			-10,				 // pcshift
			21,					 // block size
			18025,			 // number of filter entries
			false,			 // hash taken
